...
export default function Home() {
  ...
  const videoLeadFrame = videoResult?.matchedFrames[0] ?? null
      ...
      {cameraMode ? (
        ...
        <>
          <section className="mt-6 rounded-[30px] border border-white/12 bg-white/[0.06] p-6 shadow-[0_22px_80px_rgba(2,8,23,0.45)]">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h3 className="text-lg font-semibold text-white">扫描统计与策略</h3>
              </div>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              ...
              <InfoItem label="总扫描耗时" value={formatSeconds(videoResult.aggregateTimings.totalPipelineSec)} />
            </div>
          </section>
          ...
          {videoLeadFrame ? (
            <>
              <div className="mt-6">
                <ResultOverview result={videoLeadFrame} />
              </div>

              <div className="mt-6">
                <TimingComparison result={videoLeadFrame} />
              </div>
            </>
          ) : (
            <section className="mt-6 rounded-[30px] border border-white/12 bg-white/[0.06] p-6 shadow-[0_22px_80px_rgba(2,8,23,0.45)]">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.28em] text-cyan-200/70">后续动作</p>
                  <h3 className="text-lg font-semibold text-white">当前没有可复核的命中帧</h3>
                </div>
                <StatusChip label="建议换一段更清晰的视频" tone="warn" />
              </div>
              <p className="mt-4 text-sm leading-7 text-slate-300">
                这次扫描没有找到同时包含车辆与可识别车牌的帧。可以优先尝试光照更稳定、车辆更靠近镜头或视频分辨率更高的样本。
              </p>
            </section>
          )}
        </>
