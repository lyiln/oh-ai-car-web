...
async def infer_video(
    ...
    try:
        ...
        car_positive_frame_count = 0
        gated_plate_frame_count = 0
        ocr_attempt_frame_count = 0

        for frame_info in extracted_frames:
            frame_path = Path(str(frame_info["framePath"]))
            frame_run_root = run_root / "frames" / f"frame_{int(frame_info['sampleIndex']):04d}"
            payload = INFERENCE_RUNTIME.run(frame_path, frame_run_root, profile="video")
            ...
            frame_stage_timings = frame_response.get("stageTimings", {})
            aggregate_total_sec += float(frame_stage_timings.get("total_pipeline_sec", 0.0) or 0.0)
            if frame_response["carDetected"]:
                car_positive_frame_count += 1
            if int(frame_stage_timings.get("gated_plate_candidate_count", 0) or 0) > 0:
                gated_plate_frame_count += 1
            if int(frame_stage_timings.get("ocr_attempt_count", 0) or 0) > 0:
                ocr_attempt_frame_count += 1
        ...
        response = {
            ...
            "summary": (
                ...
                f"检测到车辆 {car_positive_frame_count} 帧，"
                f"通过完整车牌门控 {gated_plate_frame_count} 帧，"
                f"最终命中车辆+有效车牌 {len(matched_frames)} 帧。"
            ),
            "sampling": video_meta,
            ...
            "frameGateStats": {
                "carPositiveFrameCount": car_positive_frame_count,
                "gatedPlateFrameCount": gated_plate_frame_count,
                "ocrAttemptFrameCount": ocr_attempt_frame_count,
            },
