...
class RuntimeConfig:
    ...
    video_plate_aspect_ratio_max: float = 6.5
    video_edge_margin_ratio: float = 0.015
    video_max_ocr_candidates: int = 1
...
class WebInferenceRuntime:
    ...
    def run(self, source_image: Path, run_root: Path, profile: str = "image") -> dict[str, Any]:
        with self.lock:
    ...
    def _run_locked(self, source_image: Path, run_root: Path, profile: str = "image") -> dict[str, Any]:
        ...
        if primary_detection is not None:
            ...
            raw_plate_detection_count = len(plate_detections_roi)

            gate_start = time.perf_counter()
            candidate_detections = self._filter_plate_candidates(
                detections=plate_detections_roi,
    ...
    def _filter_plate_candidates(
        ...
        roi_h = max(int(roi_shape[0]), 1)
        roi_w = max(int(roi_shape[1]), 1)
        ...
        for detection in detections:
            ...
            det_conf = float(detection.get("confidence", 0.0) or 0.0)
            aspect_ratio = width / max(height, 1)
            area_ratio = (width * height) / max(roi_w * roi_h, 1)

            keep_candidate = True
            completeness_bonus = 0.0
            if profile == "video":
                margin_x = max(int(round(roi_w * self.config.video_edge_margin_ratio)), 2)
                margin_y = max(int(round(roi_h * self.config.video_edge_margin_ratio)), 2)
                touches_edge = (
                    x1 <= margin_x
                    or y1 <= margin_y
                    or x2 >= roi_w - margin_x
                    or y2 >= roi_h - margin_y
                )
                keep_candidate = (
                    det_conf >= self.config.video_plate_conf_thres
                    and area_ratio >= self.config.video_min_plate_area_ratio
                    and self.config.video_plate_aspect_ratio_min <= aspect_ratio <= self.config.video_plate_aspect_ratio_max
                    and not touches_edge
                )
                completeness_bonus = 0.2 if not touches_edge else 0.0

            if not keep_candidate:
                continue

            score = det_conf + area_ratio * 8.0 + completeness_bonus
            candidate = dict(detection)
            candidate["aspect_ratio"] = round(aspect_ratio, 6)
            candidate["area_ratio"] = round(area_ratio, 6)
            scored_candidates.append((score, candidate))

        scored_candidates.sort(key=lambda item: item[0], reverse=True)
        filtered = [candidate for _, candidate in scored_candidates]

        if profile == "video":
            return filtered[: max(1, self.config.video_max_ocr_candidates)]
        return filtered
