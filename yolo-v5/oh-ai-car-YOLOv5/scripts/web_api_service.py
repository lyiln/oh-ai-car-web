...
def build_runtime_paths(project_root: Path, python_executable: Path) -> RuntimePaths:
    ...
    plate_weights = next((path for path in plate_candidates if path.is_file()), plate_candidates[0])
    car_candidates = [
        project_root / "weights" / "car_bdd100k_mini_v1_best.pt",
        project_root / "weights" / "yolov5s.pt",
    ]
    car_weights = next((path for path in car_candidates if path.is_file()), car_candidates[0])
    return RuntimePaths(
