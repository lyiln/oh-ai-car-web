...
MISSING_PROVINCE_PATTERN = re.compile(r"^[A-Z][A-Z0-9]{5}$")
DEFAULT_PROVINCE_HINT = "皖"
...
def infer_missing_province_text(text: str) -> str | None:
    compact = compact_plate_text(text)
    if not MISSING_PROVINCE_PATTERN.match(compact):
        return None
    return f"{DEFAULT_PROVINCE_HINT}{compact[0]}·{compact[1:]}"
