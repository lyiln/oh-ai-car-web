...
export function WhitelistPage() {
  ...
  const refresh = async () => {
    if (!selectedId) return setEntries([]);
    setEntries(await opsClient.whitelist(selectedId));
  };
  ...
  const add = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await opsClient.addWhitelist(selectedId, {
  ...
  const importCsv = async (event: React.FormEvent) => {
    ...
    try {
      if (!selectedId) throw new Error('请先选择设备');
      const result = await opsClient.importWhitelist(selectedId, csv);
  ...
  return (
    <div className="page">
      <header className="page-header">
        <div>
          <h1>白名单管理</h1>
          <p>当前设备：{selectedDevice?.name ?? '未选择'}；维护合法车辆白名单，支持单条新增与 CSV 批量导入</p>
        </div>
        <div className="button-row">
          <button type="button" className="primary" disabled={!selectedId} onClick={() => setAddOpen(true)}>新增白名单</button>
          <button type="button" className="secondary" disabled={!selectedId} onClick={() => setImportOpen(true)}>批量导入</button>
        </div>
      </header>
