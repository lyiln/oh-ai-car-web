...
export function ResponseTasksPage() {
  ...
  const confirm = async (id: string) => {
    ...
    try {
      ...
      setMessage(result.assignedVehicleId ? `已确认并分配至车辆 ${result.assignedVehicleId.slice(0, 8)}` : '已确认，当前没有安全可用车辆，请稍后重试分配');
      await refresh();
    }
  ...
  return <div className="page">
    ...
    <section className="panel">
      {items.length === 0 ? <div className="empty-state">暂无符合上门条件的违规记录</div> : <table>
        <thead><tr><th>车牌/位置</th><th>住户目的地</th><th>执行车辆</th><th>状态</th><th>AI/规则建议</th><th>操作</th></tr></thead>
