...
/** Legacy local gateway console (routes `/connect` and `/remote`). */
export function ClassicConsole() {
  ...
  const connect = async () => {
    try { saveSettings(config); await client.open(); await client.send('connect', config); setError(null); }
  ...
  return (
    <main className="classic-console">
      <header>
        <div>
          <p className="eyebrow">OH AI CAR</p>
          <h1>小车控制台</h1>
        </div>
        <ConnectionStatus
          connected={state.connected}
          ownsControl={state.ownsControl}
          controlAvailable={state.controlAvailable}
          error={error}
          target={state.target ? `${state.target.host}:${state.target.tcpPort}` : null}
        />
      </header>
      <div className="dashboard">
        <aside>
          <ConnectionSettings
            config={config}
            configDisabled={state.connected && state.ownsControl}
            connectDisabled={state.connected || !state.controlAvailable}
            disconnectDisabled={!state.connected || !state.ownsControl}
            onChange={changeConfig}
            onConnect={connect}
            onDisconnect={disconnect}
          />
          <MediaControls disabled={disabled} pending={mediaPending} recording={recording} onPhoto={takePhoto} onToggleRecording={toggleRecording} />
          <TrackingToggle disabled={disabled} pending={trackingPending} enabled={tracking} onChange={changeTracking} />
        </aside>
        <section className="control-area">
          <VideoPanel host={config.host} port={config.videoPort} />
