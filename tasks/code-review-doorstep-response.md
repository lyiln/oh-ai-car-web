...
评审对象：commit `7ebcabc` 的“乱停车识别—上门处置”实现
...
## 集成测试稳定性复审（2026-07-12）

结论：**Approved with Notes**。

- 遥测轨迹测试改用当前 24 小时查询窗口内的时间点，避免测试数据自然过期后
  出现“写入成功、轨迹为空”的假失败。
- 每个集成用例开始前恢复管理员测试凭据；上门处置场景不再依赖前序资料修改
  用例留下的密码，已确认可独立运行，也可在完整 10 项 PostGIS 套件中运行。
- 测试仍共享同一临时数据库；后续新增用例不得依赖其他用例创建或修改的可变
  账号、凭据或业务数据。
...
- Web 已为 `assigned/navigating/arrived` 提供 `cancellation_requested` 与设备
  `stop_confirmed` 协议；ROS 适配器仍必须实际执行 Nav2 cancel 并验证零速度，
  不能把 API 状态变化当作物理停车证据。
...
- PostGIS 集成测试 10 项通过，其中包含上门任务确认、领取、到达、留证、
  完成及事件幂等流程。
- 新增测试已覆盖 Hub 跨车辆隔离、终态失败覆盖、分配失败恢复、重复分配、
  重复取消和零速度停止确认；仍未覆盖 ROS/Nav2 或真实车辆行为。
