...
©
### 阶段 1.5：核验 Docker 运行时并冻结导航架构（发现后的新增门禁）

2026-07-12 和 2026-07-14 的只读发现确认 Jetson 宿主机缺失 `/opt/ros` 和 `ros2`，
但已安装 `icar/ros-foxy:1.0.2` 和 `yahboomtechnology/ros-foxy:5.0.1` Docker
镜像，相关容器发现时均停止。车端 ROS 运行时应先按厂商 Docker 路线核验，不能据此
假设当前 Nav2 可以启动；候选地图 YAML 仍引用不存在的 `/root/...` 图像路径。

在获得单独批准前，不安装软件、不修改 Jetson 文件、不启动 Docker、底盘或传感器节点。
获批后的非运动工作必须：

1. 重新核验 Orbbec USB 设备号；为导航容器修复 `run_docker_autodrive.sh` 中过期的
   `001/009` 映射，并创建新容器、更新旧容器别名。雷达避障容器使用当前匹配
   `001/011`、`001/013` 的 `run_docker.sh`。
2. 在已选 Docker 容器中确认 ROS 2 CLI 与工作区 setup 可用；不得把宿主缺失
   `/opt/ros` 当作需要重装 ROS 的证据。
3. 在 **Nav2 `NavigateToPose`** 与现有 **RTAB-Map 导航接口** 中选择一个唯一的
   scheduler 目标接口；未作选择前不得实现或测试调度器。
4. 选择一个实际可加载的 map YAML/PGM 对，修复其路径并给出不可变 `mapVersion`。
5. 在轮子架空或断驱动状态下启动最小 graph，只读记录 `cmd_vel`、`vel_raw`、
