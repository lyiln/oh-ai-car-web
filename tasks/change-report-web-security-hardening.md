...
- `npm test`：通过（shared 5、gateway 17、frontend 30、backend 51）。
...
- `npm run test:deploy-live`：通过；Docker Compose 完整构建并启动 Web、backend 和 PostGIS，登录、车辆创建及认证/未认证 WebSocket 冒烟验证成功，临时容器和卷已清理。
